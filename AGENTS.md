...
- Codex automationはJST `13:00-13:10` と `15:30-15:40` の許可窓で起動する。worker側の時間ゲートは既定 `PREMIUM_ALLOWED_JST_HOURS=13,15` / `PREMIUM_ALLOWED_JST_MINUTES=13:00-13:10,15:30-15:40` とし、対象時間以外は即終了する。抽出対象は既定で `signal_type=BOTTOM`、`received_at` 新しい順、ロック粒度はシンボルではなく `alert_id` 単位。同一シンボルでも別 `alert_id` は別アラートとして扱い、一度投稿または手動ロックした `alert_id` は再選択しない
...
- プレミアムEmbedの `開示リンク` はPDF、TDnet `td_download.cgi`、IRBANK個別開示ページ、会社/PRの個別開示詳細ページなど、資料または開示への直リンクだけにし、IR一覧・会社概要・ニュース一覧などの参照ページは `Sources` に分ける。`開示リンク` は1件に制限せず、`足元材料` / `ファンダ要点` / `注意点` の分析に使った直近重要開示（決算、業績修正、自社株買い、配当/資本政策、M&A/提携、固定資産譲渡/特別利益、月次、規制/ガバナンスなど）は検証できた分を漏れなく載せる。リンクラベルは `開示1` / `出典1` / `会社IR` のような汎用名ではなく、実際の資料タイトルまたはページタイトルにする。本文は日本語で、各分析欄に日付・数値・事業ドライバー・確認点を含め、1行メモのような薄い要約にしない。`足元材料` は調査ログや開示タイトルの羅列ではなく、最新重要開示の日付・材料・数値・確認点を短い時系列で書き、`ファンダ要点` と同じ文を繰り返さない
- プレミアム分析文は銘柄名だけ差し替えられる汎用テンプレにしない。`事業概要` は実際の事業・主力サービス・顧客層を書く。「開示資料で確認できる主要サービス・製品を中心に事業を展開する上場企業」「売上成長、利益率、資本政策、事業提携のどれに効くか」のような実体説明を避けた文は禁止。`足元材料` はその開示が当該銘柄のどのKPIやリスクに効くか、`ファンダ要点` はARR/解約率/ARPU・既存店売上/粗利率・受注残/稼働率・新株予約権の行使ペース/希薄化・施設稼働率/借入条件など銘柄実態に合う指標、`注意点` は開示固有の未確認点（希薄化、契約金額、稼働率、受注残、統合費用、ガバナンスなど）を書く
- 開示が本当に少ない銘柄では、公式IR/IRBANKを確認したうえで新しい個別開示がないことを本文に明記すれば、古い公式開示を補助的に `開示リンク` へ置いてよい。ただし調査/フォローアップ/新規上場レポート、社長名鑑、媒体記事などの代理資料を足元材料の代替として `開示リンク` に置かない。POSTEDログの `Reason` は空欄にせず、`材料インパクト` とファンダ要点から一文総評を残す
- IRBANK個別開示ページを `開示リンク` に使う場合、そのページ内に `f.irbank.net/pdf/...pdf` または `f.irbank.net/pr/...pdf` が確認できるならPDF直リンクを優先する。プレミアムログの `Reason` は、Discordの投稿URLが取得できた場合は一文総評をMarkdownリンク化して記録する
- プレミアム投稿ログ用スプレッドシートへ記録する場合、Discord投稿ごとにSheets APIへ個別書き込みせず、1回のworker実行分をまとめて追記し、Sheets 429/5xx はバックオフして再試行する。ログ記録は補助経路なので、ログ用Sheetsの429でDiscord投稿を二重化・中断しない
