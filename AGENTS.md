...
- 開示の重要性はタイトル固定キーワードで判定しない。最新開示は本文/PDFを読み、重要なら `足元材料` / `ファンダ要点` / `注意点` / `開示リンク` に反映し、軽微・定型・主材料でない場合も内容を読んだうえで「事業KPIや資金繰りへの直接影響は限定」など銘柄固有に扱う。
- 定例的なコーポレートガバナンス報告書、通常の人事・組織変更、招集通知、独立役員届出書などは、最新開示として必ず確認するが、内容が本当に統治リスク・資本政策・上場維持・M&A・事業運営に効かない限り `材料インパクト` や `足元材料` の主材料にしない。大量件数時でも「最新開示は管理・体制面が中心」「体制更新は管理面への影響が中心」のような薄い本文でdry-runを通そうとしない。
- 直近45日内でレポート主材料より新しい開示が見えている場合は、古い開示だけで分析を通さない。新しい開示を読んで日付つきで本文または `開示リンク` に入れ、古い開示を主材料にするならその理由を本文で説明する。
- 公式IRライブラリに新しい四半期決算、月次、業績予想修正、固定資産譲渡/特別利益、株主還元方針などがある場合は、古い年度決算説明資料だけで足元材料を作らない。
- 月次開示を扱う場合は、PDF/本文内の全店・既存店・客数・客単価・売上高・EC・セグメント別数値を読み、本文に実数を入れて強弱判定する。タイトルや「月次動向」だけで「弱い」「好調」を書かない。複数セグメントが逆方向なら、どこが好調/不調かを分けて書く。
...
- `材料インパクト` には `YYYY-MM-DD` の日付を入れず、材料/イベントと事業影響から直接書く。日付は `足元材料` と `開示リンク` に回す。
- `材料インパクト` には `PDF本文でも確認`、`主要損益項目を確認`、`次回開示で確認する局面` のような調査手順や広いチェックリスト文言を入れない。
- `材料インパクト` は開示タイトルの途中切り貼りで作らない。`開示は...を含み`、`...に関するを含み`、`...ならびを含み`、`...に関する` / `...について` で終わる断片文は禁止し、事業影響を自然な日本語で書く。
- `材料インパクト` では適時開示タイトルをそのまま貼らない。`...` / `…` の省略、`[Updated]`、`Notice`、`Summary`、英語開示タイトルの断片、`に関するお知らせ` / `について` などのタイトル文言を残さず、材料・イベントと事業ドライバーへの効き方を完全文で書く。
- `材料インパクト` の `根拠要約` は前回良例と同じく「材料/イベント + 事業への効き方」を結論化する。`...が支えです`、`...が焦点です`、`...が重いです` のような薄い締め方で終えず、売上、利益率、希薄化、資金繰り、受注、稼働率、還元など銘柄固有ドライバーへの影響を書く。
...
- `Sources` は参照ページを1 URL 1回だけ載せる。同じIRBANK/Yahoo/会社IR一覧URLを開示タイトル別に複数行へ増やさず、開示ごとの根拠は `開示リンク` に載せる。
- `開示リンク` は1件に制限しない。
- `足元材料` / `ファンダ要点` / `注意点` の分析に使った直近重要開示は、検証できた分を漏れなく載せる。
- `足元材料` / `ファンダ要点` / `注意点` でIR/開示タイトル、公式PR、表彰、ガバナンス、招集通知などに言及した場合は、主材料でなくても該当資料または公式個別ページを必ず `開示リンク` に含める。
...
- 1行メモのような薄い要約にしない。
- `足元材料` / `ファンダ要点` / `注意点` はバッチ内で同じ本文を使い回さない。`決算・還元・提携などが収益性、資本効率、事業進捗へ与える実質影響が焦点`、`株主還元や資本効率方針はROE` のような汎用バケット文は禁止し、銘柄固有のKPI・事業ドライバー・リスクを書く。
- ファンダ分析の品質基準は `premium_worker/FUNDAMENTAL_EXAMPLES.md` を参照する。ただし固定フォーマットにはせず、銘柄・材料・開示内容に合わせて構成と重点を変える。
- 大量claimでもファンダ分析を圧縮しない。10件以上のバッチでは銘柄ごとに材料開示、開示リンク数、Sources数、銘柄固有KPI、注意点を内部確認し、workerのlarge-batch品質ゲートをdry-runで通してから投稿する。13:05/15:36の両方で同じ基準を使う。
- `足元材料` は調査ログや開示タイトルの羅列ではなく、最新重要開示の日付・材料・数値・確認点を短い時系列で書く。
- `足元材料` と `ファンダ要点` で同じ文を繰り返さない。
- プレミアム分析文は銘柄名だけ差し替えられる汎用テンプレにしない。
- `事業概要` は実際の事業・主力サービス・顧客層だけを書く。月次売上、来場者数、客単価、粗利率、稼働率、受注残、資金繰り、配当、自社株買いなど収益・KPI・資本政策の話は `ファンダ要点` に書く。
...
- `今回の開示では...具体的に追います`、`どこに効くか`、`見ます`、`見たい`、`見極めたい`、`読み取れる結果は` のような調査予定・前置き表現は禁止。欄名で分析欄だと分かるため、前置きなしでKPI・利益率・資金繰り・希薄化・継続性への具体的な影響を書く。
- `足元材料` は、その開示が当該銘柄のどのKPIやリスクに効くかを書く。
- `足元材料` にカレンダー日付を書くときは `YYYY-MM-DD` ではなく `5月8日` のような `M月D日` 形式にする。`2026年3月期` のような会計期間と、`開示リンク` ラベルの `YYYY-MM-DD 開示タイトル(hh:mm)` は維持してよい。
- `ファンダ要点` は銘柄実態に合う指標を書く。
  ...
  - 借入条件
  - その他、銘柄固有の重要KPI
- `ファンダ要点` は銘柄名や証券コードで始めない。Embedタイトルで銘柄は分かるため、KPI、利益率、資金繰り、希薄化、受注、稼働率などの評価軸から直接書き始める。
- `注意点` は開示固有の未確認点を書く。
- `注意点` は `1234では...` のように証券コードで始めない。Embedタイトルで銘柄は分かるため、リスク・KPI・未確認点から直接書き始める。
...
- `insufficient verified sources` の `fail --alert-id` を多数連続実行してbatchガードを回避しない。workerは短時間の連続stubを拒否するため、失敗銘柄だけを根拠付きレポート修復または真のソース不足として個別に処理する。
...
| `HISTORICAL_VOLUME_REPAIR_PAUSE_V1` | 手動停止またはUrlFetch日次上限待機の理由。日次上限時は再開予定時刻も保持する |
...
- `repairHistoricalOhlcvVolumes()` / 手動の `resumeHistoricalOhlcvVolumeRepair()` で開始したactiveジョブだけがfetchする。開始後の再開は自動で行う。
- 13:21/15:51 OHLCV本体がactiveの間はfetchせず30分後へ延期し、完了するまで同じ判定を繰り返す。
- `Service invoked too many times for one day: urlfetch` ではカーソルを保持し、約24時間はfetchせず待機してから1回だけ自動再開する。再開時に本体がactiveなら30分延期判定へ移る。
- 手動pause中またはactiveでない保存stateは `runGlobalExecutionWatchdog` から復活させない。日次上限の自動再開予定時刻を過ぎたactiveジョブだけは、再開トリガー欠落時にウォッチドッグで復旧する。
...
- `runGlobalExecutionWatchdog` は5分ごとに、固定6トリガーの欠落と、未完了状態があるのに対応する再開トリガーがない処理を1回最大3件まで復旧する。ウォッチドッグ自身からシート、通知内容、処理順を変更しない。
- 長時間処理は重い処理へ入る前に、再開状態または専用active markerを保存し、6.5分後の先行保険トリガーを置く。正常完了時に状態・marker・保険トリガーを一緒に消す。
- ワンショット再開トリガーを置換するときは、`scheduleRecoveryTriggerReplacing_()` を使って新しいトリガーの作成成功後に古い同名トリガーを削除する。`deleteTriggersByHandler_()` → `ScriptApp.newTrigger()` の順で置換しない。
- 手動停止、非再試行エラー、書き込み検証失敗、dry-runなど自動再開してはいけない経路では、対応するactive markerを必ず消す。単なる残存stateを根拠に意図的停止を復活させない。
- 監視トリガーだけを追加・修復する場合は `setupGlobalExecutionWatchdogTrigger()` を実行する。動的チェーン稼働中に `setupAllTriggers()` を代用しない。
- 共通リトライ基盤を変更したら `node --test tests/global_auto_retry.test.mjs` を実行し、直接の `ScriptApp.newTrigger` が中央ワンショット生成、共通ウォッチドッグ、固定6トリガー以外へ増えていないことを確認する。
