...
- プレミアム分析文は銘柄名だけ差し替えられる汎用テンプレにしない。`事業概要` は実際の事業・主力サービス・顧客層、`足元材料` はその開示が当該銘柄のどのKPIやリスクに効くか、`ファンダ要点` はARR/解約率/ARPU・既存店売上/粗利率・受注残/稼働率・新株予約権の行使ペース/希薄化・施設稼働率/借入条件など銘柄実態に合う指標、`注意点` は開示固有の未確認点（希薄化、契約金額、稼働率、受注残、統合費用、ガバナンスなど）を書く
