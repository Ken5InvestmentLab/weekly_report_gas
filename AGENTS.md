...
- プレミアムEmbedの `開示リンク` はPDFやTDnet `td_download.cgi` など資料ファイル直リンクだけにし、IR一覧・会社概要・ニュース一覧などの参照ページは `Sources` に分ける。リンクラベルは `開示1` / `出典1` / `会社IR` のような汎用名ではなく、実際の資料タイトルまたはページタイトルにする。本文は日本語で、各分析欄に日付・数値・事業ドライバー・確認点を含め、1行メモのような薄い要約にしない
