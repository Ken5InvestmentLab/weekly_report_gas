...
- プレミアムEmbedの `開示リンク` はPDF、TDnet `td_download.cgi`、IRBANK個別開示ページ、会社/PRの個別開示詳細ページなど、資料または開示への直リンクだけにし、IR一覧・会社概要・ニュース一覧などの参照ページは `Sources` に分ける。リンクラベルは `開示1` / `出典1` / `会社IR` のような汎用名ではなく、実際の資料タイトルまたはページタイトルにする。本文は日本語で、各分析欄に日付・数値・事業ドライバー・確認点を含め、1行メモのような薄い要約にしない。`足元材料` は調査ログや開示タイトルの羅列ではなく、最新重要開示の日付・材料・数値・確認点を短い時系列で書き、`ファンダ要点` と同じ文を繰り返さない
